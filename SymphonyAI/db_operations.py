...
def create_db_from_dataset(dbname, tbl_name, dataset):
    '''
    create db, table and insert values to table
    '''
    conn =  sqlite3.connect(dbname)
    cur = conn.cursor()                                 
 
    columns = ','.join(['?'] * len(dataset.keys()))        
    data = [tuple(t) for t in zip(*dataset.values())]
    
    cur.execute("drop table if exists %s" % tbl_name)
    
    col_str = '"' + '","'.join(dataset.keys()) + '"'
    
    cur.execute("create table %s (%s)" % (tbl_name, col_str))
    
    cur.executemany("insert into %s values(%s)" % (tbl_name, columns), data)
    
    conn.commit()
    conn.close()

############################BELOW is CODE to read CSV file and put data into database####################################
